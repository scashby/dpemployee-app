...
const ScheduleEditor = () => {
  ...
  const formatLabel = (startDate) => {
    ...
    return `Week of ${start.toLocaleDateString()} through ${end.toLocaleDateString()}`;
  };
        ...
        <tbody>
          {weekData?.employees?.map((emp, i) => (
            <tr key={i} className="border-t">
              <td className="p-2">
                <input value={emp} onChange={e => updateEmpName(i, e.target.value)} className="w-full border px-1" />
              </td>
